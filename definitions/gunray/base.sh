...
mkdir -p $VAGRANT_LOG_DIR
echo "built with veewee v$(cat .veewee_version) and vbox v$(cat .vbox_version), on $(date)." > $VAGRANT_LOG_DIR/veewee.log
...
DEBIAN_RELEASE_STABLE=stretch
DEBIAN_RELEASE_TESTING=buster
mv /etc/apt/sources.list /etc/apt/sources.list~
cat > /etc/apt/sources.list.d/official.cdn.list <<EOF
deb ${DEBIAN_MIRROR} ${DEBIAN_RELEASE_STABLE} main
# deb ${DEBIAN_MIRROR} ${DEBIAN_RELEASE_TESTING} main
# deb ${DEBIAN_MIRROR} sid main
deb ${DEBIAN_MIRROR}-security ${DEBIAN_RELEASE_STABLE}/updates main
# deb ${DEBIAN_MIRROR}-security ${DEBIAN_RELEASE_TESTING}/updates main
EOF
...
cat <<EOF > /etc/apt/apt.conf.d/30proxy
# Acquire::http::proxy "http://192.168.1.1:8000/";
# Acquire::https::proxy "http://192.168.1.1:8000/";
EOF
...
aptitude -q2 -y install bzip2 ca-certificates curl file git htop less lsb-release lsof net-tools nfs-common patch python rsync ruby unzip vim-nox zsh
